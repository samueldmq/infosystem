...
_HTML_EMAIL_TEMPLATE = """
    ...
    <p>Você acaba de ser cadastrado no portal da Distribuidora de Alimentes
    Seridó LTDA.</p>
    <p>Para ter acesso ao sistema você deve clicar no link abaixo para
    confirmar esse email e criar uma senha.</p>

    <div style="width: 100%; text-align: center">
        <a href="{reset_url}">Clique aqui para CONFIRMAR o email e CRIAR uma
        senha.</a>
    </div>
"""
...
def send_email(token_id, reset_user):
    try:
        ...
        default_email_subject = (
            'PORTAL OBJETO RELACIONAL - CONFIRMAR email e CRIAR senha')
        ...
        infosystem_email_use_sandbox = bool(os.environ.get(
            'INFOSYSTEM_EMAIL_USE_SANDBOX', default_email_use_sandbox))
...
class Routes(operation.Operation):
    ...
    def do(self, session, user_id, **kwargs):
        ...
        policies_capabilitys_id = [p.capability_id for p in policies
                                   if p.role_id in user_roles_id]

        user = self.manager.api.users.list(id=user_id)[0]
        capabilities = self.manager.api.capabilities.list(
            domain_id=user.domain_id)

        policy_capabilities = [c for c in capabilities
                               if c.id in policies_capabilitys_id]

        # NOTE(samueldmq): if there is no policy for a capabiltiy, then it's
        # open! add it too!
        restricted_capabilities = [p.capability_id for p in policies]
        open_capabilities = [c for c in capabilities
                             if c.id not in restricted_capabilities]

        user_routes = [self.manager.api.routes.get(id=c.route_id)
                       for c in (policy_capabilities + open_capabilities)]
