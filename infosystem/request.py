...
class Request(flask.Request):
    ...
    def url(self):
        ...
        path_bits = ['<id>' if self._check_uuid4(i) else i
                     for i in path_info.split('/')]
...
class RequestManager(object):
    ...
    def before_request(self):
        ...
        # Short-circuit if accessing the root URL, which will just return
        # the version
        ...
        if route.bypass:
            ...
            # TODO(samueldmq): THIS SHOULD BE RE-ENABLED, DISABLED FOR NOW
            # FOR /FOTOS
            # else:
            #     return flask.Response(response=None, status=400)
        else:
            ...
            if token_id:
                ...
                # TODO(samueldmq): sysadmin won't provide a domain, so
                # capabilities will be empty treat this case here once we
                # support sysadmin
                ...
                intersection = set(user_role_ids).intersection(
                    policy_role_ids)
                if not intersection:
                    return flask.Response(response=None, status=401)
            else:
